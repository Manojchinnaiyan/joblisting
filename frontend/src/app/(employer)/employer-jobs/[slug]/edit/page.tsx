...
import { useEmployerJob, useUpdateJob } from '@/hooks/employer/use-employer-jobs'
...
export default function EditJobPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    if (job) {
      form.reset({
        ...
        experience_level: job.experience_level as JobFormData['experience_level'],
        workplace_type: job.workplace_type as JobFormData['workplace_type'],
        ...
        application_deadline: job.expires_at ? job.expires_at.split('T')[0] : '',
  ...
  const onSubmit = async (data: JobFormData) => {
    ...
    router.push(`/employer-jobs/${slug}`)
  }
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        placeholder="List benefits like health insurance, 401k, equity, etc..."
                        rows={3}
                        {...field}
                      />
                    </FormControl>
          ...
          <div className="flex justify-end gap-4">
            <Button type="button" variant="outline" asChild>
              <Link href={`/employer-jobs/${slug}`}>Cancel</Link>
            </Button>
