...
interface InitialCompaniesData {
  companies: Company[]
  pagination: PaginationType
}

interface CompaniesPageClientProps {
  initialData: InitialCompaniesData | null
}
...
export function CompaniesPageClient({ initialData }: CompaniesPageClientProps) {
  ...
  useEffect(() => {
    // Only fetch if user has searched or changed page, otherwise use initial data
    if (hasSearched || currentPage > 1 || !initialData) {
      fetchCompanies()
    }
  }, [fetchCompanies, hasSearched, currentPage, initialData])
