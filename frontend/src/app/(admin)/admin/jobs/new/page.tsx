...
import { useCreateAdminJob } from '@/hooks/admin'
import { useAdminCategories } from '@/hooks/admin/use-admin-categories'
import { AdminCreateJobData } from '@/lib/api/admin/jobs'
...
const jobSchema = z.object({
  ...
  status: z.enum(['ACTIVE', 'DRAFT', 'PENDING_APPROVAL']).optional(),
})
...
export default function CreateJobPage() {
  ...
  const onSubmit = async (data: JobFormData) => {
    const jobData: AdminCreateJobData = {
      title: data.title,
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Create a new job listing as an admin
          </p>
        </div>
      </div>
